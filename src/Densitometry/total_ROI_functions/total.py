        ...
        for roi_contour in ds.ROIContourSequence:
            ...
            if str(roi_number) == str(roi_contour.ReferencedROINumber):
                if "ContourSequence" in roi_contour:
                    ...
                    print(
                        f"ContourSequence not in ROIContourSequence: probably {roi_name} is empty."
                    )
                    break
            else:
...
def find_rt_st(ct_path: Path, rt_kind: str, list_roi: list) -> tuple[str, Path] | None:
    ...
    try:
        ...
        path_rt_structures = [
            path_rt_st
            for path_rt_st in list(Path(ct_path).parents[2].glob(f"**/*{rt_kind}*"))
            if path_rt_st.is_dir() == False
        ]
...
def parallel_fun(pz: int, input_parallel_dictionary: dict) -> tuple[pd.DataFrame, list]:
    ...
    if str(ID) not in ID_problems:
        ...
        try:
            ...
            old_sp = np.array(
                [
                    df_py.loc[pz, "VoxelSpacingX"],
                    df_py.loc[pz, "VoxelSpacingY"],
                    df_py.loc[pz, "VoxelSpacingZ"],
                ]
            )
            ...
            if np.all(old_sp == new_sp):
                ...
                HU_ROI, counts_ROI = info.ROI_ok(
                    ct_path, ROI_path, ROI_founded, show_info_all, slice=40
                )
                # Extract information
                stats_df = histo.features_ROI(
                    ID,
                    HU_ROI,
                    counts_ROI,
                    new_sp,
                ...
                dir_compare_ct_res = (
                    Path(directory_out) / "Total_ROI" / "To_be_resampled" / "Compare_histo"
                )
                ...
                HU_ROI_res, counts_ROI_res = info.ROI_res(
                    ct_path, ROI_path, new_sp, ROI_founded, show_info_all, resampler, slice=100
                )
                # Extract information
                stats_df = histo.features_ROI(
                    ID,
                ...
                compare = histo.compare_histo_res(
                    HU_ROI_res,
                    counts_ROI_res,
                    HU_ROI,
                    counts_ROI,
...
def res_and_create_histo(
    ...
    results = Parallel(n_jobs=N_jobs, timeout=None)(
        delayed(parallel_fun)(pz, input_parallel_dictionary) for pz in range(len(df_py))
    )
...
def no_res_and_create_histo(
    ...
    # Parallel function
    results = Parallel(n_jobs=N_jobs, timeout=None)(
        delayed(no_res_parallel_fun)(pz, input_parallel_dictionary) for pz in range(len(df_py))
    )
