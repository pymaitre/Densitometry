...
def main(conf):
    ...
    df_py = info_dcm.find_ct_info(
        directory_dcm_out, image_modality, py_patient_file=py_patient_file
    )
    ...
    if flag_resampling:
        ...
        if (
            flag_new_spacing == "min_global"
            or flag_new_spacing == "mean_global"
            or flag_new_spacing == "max_global"
        ):
        ...
        if all:
            ...
            dir_files_fin = tot.res_and_create_histo(
                df_py,
                ID_problems,
                new_sp,
                rt_kind,
        ...
        if all:
            print("Analyzing total histograms is set on: ", all)
            dir_files_fin = tot.no_res_and_create_histo(
                df_py,
                ID_problems,
                rt_kind,
                list_roi,
