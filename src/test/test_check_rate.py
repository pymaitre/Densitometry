...
def test_rate_variable(monkeypatch)->tuple[int,int,float]:
    """
...
def test_analyze_rate(reference_HU_ok: pd.Series, reference_counts_ok: pd.Series, HU_min: int, HU_max: int)->tuple[float, pd.Series, pd.Series]:
    """
...
def test_check_rate(reference_folder_files_ok: str, reference_dir_out: str, rate_over_ROI: tuple[int,int,float])->None:
    """
    ...
    path_sp=reference_dir_out/"Total_ROI"/"Histo_total_stats.xlsx"
    ...
    for diff_file in diff_files:
        ...
        #Chiedere se posso usare funzioni test dentro
        diff, HU_rate, counts_rate = test_analyze_rate(HU_ROI, counts_ROI, HU_min, HU_max)
        ...
        if diff > (rate/100.):       
            ...
            dir_histo_rate = Path(reference_dir_out) / "Over_rate_regions" / f"Region_over_{HU_min}_{HU_max}_{rate}" / f"Histograms"
            Path(dir_histo_rate).mkdir(parents=True, exist_ok=True)
        
            dir_files_rate = Path(reference_dir_out) / "Over_rate_regions" / f"Region_over_{HU_min}_{HU_max}_{rate}" / f"Files"
    ...
    if len(more_patient_stats_df_rate!=0):
        ...
        if save_rate:      
            excel_file_rate = Path(reference_dir_out) / "Over_rate_regions" / f"Region_over_{HU_min}_{HU_max}_{rate}" / f"Stats_over_{HU_min}_{HU_max}_{rate}.xlsx"
