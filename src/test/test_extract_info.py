...
@pytest.mark.parametrize("save_info_all,ID",[(True,"1"),(False,"1")])
def test_save_info_nifti(save_info_all:bool, reference_dir_out:Path, reference_CT:rsm.Image, reference_RT:rsm.RTStructureSet, ID:str):
   
   """ Test if save_into_nifti works correctly. """
   
   save_info_nifti(save_info_all,reference_dir_out,reference_CT,reference_RT,ID)
            
@pytest.mark.parametrize("save_info_all,ID",[(True,"1"),(False,"1")])
def test_save_info_nifti_no_CT(save_info_all:bool, reference_dir_out:Path,reference_RT:rsm.RTStructureSet, ID:str):
    
    """ Test if save_into_nifti raises Exception. """
...
def test_read_and_show_ct(reference_CT_path:Path, show_CT_ROI:bool, slice:int):
    ...
    """ Test if read_and_show_ct runs correctly. """
...
def test_read_and_show_RTst(reference_RT:rsm.RTStructureSet, ROI_founded:str):
    ...
    """ Test if read_and_show_RTst works correctly. """
...
def test_obtain_ROI(reference_mask:np.array, reference_CT_arr:np.array, show_CT_ROI:bool, slice:int):
    ...
    """ Test if obtain_ROI runs correctly. """
...
def test_resample(reference_CT: rsm.Image, new_x:float, new_y:float, new_z:float,resampler):
    ...
    """ Test if resample function runs correctly. """   
