...
def reference_voxelspacingx()->pd.Series:
    """VoxelSpacingX used during testing."""
...
def reference_sp()->np.array:
    """array containing the VoxelSpacing."""
    df_py=pd.read_excel(Path(Path(__file__).parent/"output_test"/"py_patient.xlsx"))
...
def reference_compare_dir()->Path:
    """Path to the compare histogram folder."""
...
def reference_RT()->rsm.RTStructureSet:
    """RTSTRUCT file used for testing."""
...
def reference_mask()->np.array:
    """Reference mask used for testing."""
    reference_path_rt=Path(__file__).parent.parent.parent/"tutorials"/"tutorial_patient"/"IBSI1_CT_phantom"/"CT"/"CT_1"/"Rtst"/"DCM_RS_00060.dcm"
    reference_CT=rsm.Image.read(Path(__file__).parent.parent.parent/"tutorials"/"tutorial_patient"/"IBSI1_CT_phantom"/"CT"/"CT_1"/"CT")
