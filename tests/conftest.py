...
def reference_file_ok() -> pd.DataFrame:
    ...
    return pd.read_excel(
        Path(__file__).parent / "output_test" / "Total_ROI" / "Files_ok" / "1.xlsx"
    )
...
def reference_HU_no_res() -> pd.Series:
    ...
    file_no_res = pd.read_excel(
        Path(__file__).parent / "output_test" / "Total_ROI" / "To_be_resampled" / "Files" / "1.xlsx"
    )
...
def reference_RT() -> rsm.RTStructureSet:
    ...
    return rsm.RTStructureSet.read(
        filename=reference_path_rt, structure_names="GTV-1", reference_image=reference_CT
    )
...
def reference_mask() -> np.array:
    ...
    reference_RT = rsm.RTStructureSet.read(
        filename=reference_path_rt, structure_names="GTV-1", reference_image=reference_CT
    )
...
def reference_dict_resampling() -> dict:
    ...
    dic = {
        "directory_histo_fin": Path(__file__).parent
        / "output_test"
        / "Total_ROI"
        / "Histograms_ok",
