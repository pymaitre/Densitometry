...
def test_over_HU_counts(
    reference_HU_ok: pd.Series, reference_counts_ok: pd.Series, HU_min: int, min_counts: int
):
...
def test_check_over(
    reference_folder_files_ok: Path, reference_dir_out: Path, delimiter_over_ROI: tuple[int, int]
):
