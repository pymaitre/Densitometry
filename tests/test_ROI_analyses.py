...
@pytest.mark.parametrize(
    "specific_flag,over_flag,rate_flag,expected",
    [
...
def test_analysis_check(
    reference_conf_ROI_analyses: dict,
    specific_flag: Any,
    over_flag: Any,
    rate_flag: Any,
    expected: tuple,
    ...
    ref_conf = modify_configuration_file(
        reference_conf_ROI_analyses,
        ["specific_ROI_analyses", "over_ROI_analyses", "rate_ROI_analyses"],
        [specific_flag, over_flag, rate_flag],
    )
...
@pytest.mark.parametrize(
    "specific_flag,over_flag,rate_flag,delimiter_specific,delimiter_rate",
    [
        (True, False, False, [100, -100, 0], [-100, 100, 0]),
        (False, False, True, [-100, 100, 0], [100, -100, 0]),
    ],
)
def test_analysis_check_invalid_delimiters(
    reference_conf_ROI_analyses: dict,
    specific_flag: bool,
    over_flag: bool,
    rate_flag: bool,
    delimiter_specific: list,
    delimiter_rate: list,
):
    ...
    ref_conf = modify_configuration_file(
        reference_conf_ROI_analyses,
        [
            "specific_ROI_analyses",
            "over_ROI_analyses",
            "rate_ROI_analyses",
            "specific_ROI_delimiter",
            "rate_ROI_delimiter",
        ],
        [specific_flag, over_flag, rate_flag, delimiter_specific, delimiter_rate],
    )
...
def test_main_empty_dir_files_fin(reference_conf_ROI_analyses: Path, reference_dir_out_empty: Path):
    ...
    conf = modify_configuration_file(
