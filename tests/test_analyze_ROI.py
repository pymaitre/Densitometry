...
def test_all_ROI(
    ...
    df_ROI, df_counts = all_ROI(
        reference_py_patient_file_dataset, ID_problems, reference_dir_out, rt_kind
    )
...
@pytest.mark.parametrize("ID_problems, rt_kind", [([], "DCM_RS")])
def test_all_ROI_exception(
    reference_py_patient_file_dataset: pd.DataFrame,
    reference_dir_out_empty: Path,
    ID_problems: list,
    rt_kind: str,
):
    ...
    df_ROI, df_counts = all_ROI(
        reference_py_patient_file_dataset, ID_problems, reference_dir_out_empty, rt_kind
    )
