...
from Densitometry.total_ROI_functions.extract_info import (
    save_info_nifti,
    ROI_ok,
    ROI_res,
    read_and_show_ct,
...
def test_save_info_nifti(
    save_info_all: bool,
    reference_dir_out: Path,
    reference_CT: rsm.Image,
    reference_RT: rsm.RTStructureSet,
    ROI_name: str,
    ID: str,
):
...
@pytest.mark.parametrize("save_info_all,ID,ROI_name", [(True, "1", "GTV-1"), (False, "1", "GTV-1")])
def test_save_info_nifti_no_CT(
    save_info_all: bool,
    reference_dir_out: Path,
    reference_RT: rsm.RTStructureSet,
    ROI_name: str,
...
def test_ROI_ok(
    ...
    HU_ROI, counts_ROI = ROI_ok(
        reference_CT_path, reference_RT_path, ROI_founded, show_CT_ROI, slice
    )
...
@pytest.mark.parametrize(
    "new_sp,ROI_founded,show_CT_ROI,resampler,slice",
    [
        ([1, 1, 3], "GTV-1", True, sitk.sitkBSpline, 0),
        ([1, 1, 3], "GTV-1", False, sitk.sitkBSpline, 0),
    ],
)
def test_ROI_res(
    reference_CT_path: Path,
    ...
    HU_ROI_res, counts_ROI_res = ROI_res(
        reference_CT_path, reference_RT_path, new_sp, ROI_founded, show_CT_ROI, resampler, slice
    )
...
@pytest.mark.parametrize("show_CT_ROI,slice", [(True, 0), (False, 0)])
def test_obtain_ROI(
    reference_mask: np.array, reference_CT_arr: np.array, show_CT_ROI: bool, slice: int
):
    """Test if obtain_ROI behaves correctly."""

    HU_ROI_no_nan, counts_ROI_no_nan = obtain_ROI(
        reference_mask, reference_CT_arr, show_CT_ROI, slice
    )
