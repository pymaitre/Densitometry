"""
Test module for total.py
"""

import os
from pathlib import Path
from Densitometry.total_ROI_functions.total import (
    get_roi_names,
    is_roi_empty,
    find_rt_st,
    parallel_fun,
    res_and_create_histo,
    no_res_parallel_fun,
    no_res_and_create_histo,
)
...
def test_is_roi_empty(reference_RT_path: Path, roi_name: str):
    """Verify if is_roi_empty checks correctly the presence of the ROI."""
...
def test_is_roi_empty_not_valid_roi(reference_RT_path: Path):
    """Verify if is_roi_empty checks correctly the absence of a missing ROI."""
...
def test_find_rt_st(reference_CT: Path, rt_kind: str, list_roi: list):
    """Test if the ROI and the Path are found."""
...
def test_parallel_fun(pz: int, reference_dict_resampling: dict):
    """Test if parallel_fun extracts correctly the statistical features."""
...
def test_parallel_fun_not_valid_pz(reference_dict_resampling: dict):
    """Consider a not valid pz for parallel_fun."""
...
@pytest.mark.parametrize("pz", [0])
def test_parallel_fun_old_sp(
    pz: int, reference_dict_resampling: dict, reference_py_patient_file_dataset: pd.DataFrame
):
    """Consider the original spacing in parallel_fun."""
...
def test_parallel_fun_mising_roi(pz: int, reference_dict_resampling: dict):
    """Consider a missing ROI as input for parallel_fun."""
...
def test_parallel_fun_ID_problem(pz: int, reference_dict_resampling: dict):
    """Assume that the patient has problems (parallel_fun)."""
...
@pytest.mark.parametrize(
    "ID_problems,new_sp,rt_kind,list_roi,show_info_all,save_info_all,N_jobs,resampler",
    [
        ([], [1, 1, 3], "DCM_RS", ["GTV-1"], True, True, 2, sitk.sitkBSpline),
        ([], [1, 1, 3], "DCM_RS", ["GTV-1"], True, False, 2, sitk.sitkBSpline),
        ([], [1, 1, 3], "DCM_RS", ["GTV-1"], False, True, 2, sitk.sitkBSpline),
        ([], [1, 1, 3], "DCM_RS", ["GTV-1"], False, False, 2, sitk.sitkBSpline),
        (["1"], [1, 1, 3], "DCM_RS", ["GTV-1"], False, False, 2, sitk.sitkBSpline),
    ],
)
def test_res_and_create_histo(
    reference_py_patient_file_dataset: pd.DataFrame,
    ID_problems: list,
    new_sp: np.array,
    rt_kind: str,
    list_roi: list,
    ...
    """Tests for res_and_create_histo function."""

    dir_files_fin = res_and_create_histo(
        reference_py_patient_file_dataset,
        ID_problems,
        new_sp,
        rt_kind,
        list_roi,
...
def test_no_res_parallel_fun(pz: int, reference_dict_no_res: dict):
    """Test if no_res_parallel_fun extracts correctly the statistical features."""
...
def test_no_res_parallel_fun_not_valid_pz(reference_dict_no_res: dict):
    """Consider a not valid pz for no_res_parallel_fun."""
...
def test_no_res_parallel_fun_mising_roi(pz: int, reference_dict_no_res: dict):
    """Consider a missing ROI as input for no_res_parallel_fun."""
...
def test_no_res_parallel_fun_ID_problem(pz: int, reference_dict_no_res: dict):
    """Assume that the patient has problems (no_res_parallel_fun)."""
...
@pytest.mark.parametrize(
    "ID_problems,rt_kind,list_roi,show_info_all,save_info_all,N_jobs",
    [
        ([], "DCM_RS", ["GTV-1"], True, True, 2),
        ([], "DCM_RS", ["GTV-1"], True, False, 2),
        ([], "DCM_RS", ["GTV-1"], False, True, 2),
        ([], "DCM_RS", ["GTV-1"], False, False, 2),
        (["1"], "DCM_RS", ["GTV-1"], True, True, 2),
    ],
)
def test_no_res_and_create_histo(
    reference_py_patient_file_dataset: pd.DataFrame,
    ID_problems: list,
    rt_kind: str,
    list_roi: list,
    ...
    """Tests for no_res_and_create_histo function."""

    dir_files_fin = no_res_and_create_histo(
        reference_py_patient_file_dataset,
        ID_problems,
        rt_kind,
        list_roi,
