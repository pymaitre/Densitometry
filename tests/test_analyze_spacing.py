"""
Test module for analyze_spacing.py
"""
...
import numpy as np
import pytest
from Densitometry.total_ROI_functions.analyze_spacing import (
    read_spacing,
    histo_spacing,
    find_global_scale,
)
...
def test_read_spacing(
    reference_py_patient_file_dataset: pd.DataFrame, reference_dir_out: Path, save_sp: bool
):
    """Verify that the identified voxel spacing is valid."""

    new_x, new_y, new_z = read_spacing(
        reference_py_patient_file_dataset, reference_dir_out, save_sp
    )
...
def test_histo_spacing(
    reference_voxelspacingx: pd.Series,
...
def test_find_global_scale(
    reference_py_patient_file_dataset: pd.DataFrame, new_spacing_approach: str
):
    """Test different new spacing approaches."""
...
def test_not_valid_scale(
    reference_py_patient_file_dataset: pd.DataFrame, new_spacing_approach: str
):
    """Test an invalid new spacing approach."""
