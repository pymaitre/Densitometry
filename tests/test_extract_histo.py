"""
Test module for extract_histo.py
"""
...
from Densitometry.total_ROI_functions.extract_histo import (
    features_ROI,
    make_histo,
    make_file,
    plot_stat,
...
@pytest.mark.parametrize(
    "ID,ROI_name,save",
    [("1", "GTV-1", True), ("1", "GTV-1", False)],
)
def test_features_ROI(
    ID: str,
    reference_HU_ok: pd.Series,
    reference_counts_ok: pd.Series,
    reference_sp: np.array,
    ROI_name: str,
    reference_dir_histo_ok: Path,
    reference_folder_files_ok: Path,
    save: bool,
):
    """Test if the function features_ROI behaves correctly."""

    stats_df = features_ROI(
        ID,
        reference_HU_ok,
        reference_counts_ok,
        reference_sp,
        ROI_name,
        reference_dir_histo_ok,
        reference_folder_files_ok,
        save,
    )
...
@pytest.mark.parametrize(
    "n_size,ROI_name,name,save", [(1, "GTV-1", "1", True), (1, "GTV-1", "1", False)]
)
def test_make_histo(
    reference_HU_ok: pd.Series,
    reference_counts_ok: pd.Series,
...
def test_plot_stat(reference_data: np.array):
    """Test for plot_stat function."""

    plot_stat(reference_data)


@pytest.mark.parametrize(
    "histo_name,region,ROI_name",
    [("1", "GTV-1", "GTV-1")],
)
def test_calculate_and_save_statistics(
    histo_name: str, region: str, reference_data: np.array, reference_sp: np.array, ROI_name: str
):
    """Verify that all the statistics are calculated."""

    stats_data = calculate_and_save_statistics(
        histo_name, region, reference_data, reference_sp, ROI_name
    )
...
def test_compare_histo_res(
    reference_HU_ok: pd.Series,
    reference_counts_ok: pd.Series,
    reference_HU_no_res: pd.Series,
    reference_counts_no_res: pd.Series,
    reference_compare_dir: Path,
    ID: str,
    save,
):
    """Generate the comparison between the histograms."""

    compare_histo_res(
        reference_HU_ok,
        reference_counts_ok,
        reference_HU_no_res,
        reference_counts_no_res,
        reference_compare_dir,
        ID,
        save,
    )
...
def test_extract_hist(
    reference_HU_ok: pd.Series, reference_counts_ok: pd.Series, n_size: int, color: str, label: str
):
    """Test if the plot is correctly created."""
